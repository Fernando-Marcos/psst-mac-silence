...
- `SilenceController.swift`: estado, activación, vigilancia continua, avisos, restauración y persistencia reversible.
...
5. Mientras el modo está activo, un ciclo local comprueba cada 400 ms el estado de mute, volumen y actividad genérica del dispositivo.
6. Si el volumen reaparece, Psst conserva el estado original de una salida nueva, vuelve a silenciarla y presenta el aviso de bloqueo.

La desactivación detiene primero la vigilancia, restaura la instantánea y abre el atajo inverso. Los dispositivos digitales que no exponen control de volumen por software pueden rechazar el cambio; Psst lo comunica sin intentar eludir macOS.
